"""Game-window capture for the no-tech.key screen mode — Windows.
...
No beamngpy, no game cooperation, no extra pip packages (raw
user32/gdi32 through ctypes). The player uses the HOOD camera in-game so
the captured view approximates a windshield mount. The window is
re-located every couple of seconds so moves and resizes are followed.

Test (game running):
    .venv\\Scripts\\python -m simsteer.io.screen_capture
"""
...
_RELOCATE_S = 2.0
_PW_CLIENTONLY = 0x1
_PW_RENDERFULLCONTENT = 0x2      # undocumented pre-1809, official since
_BI_RGB = 0
